...
/// Use `VersmConfig::default()` value if no config or args
#[test]
fn start_no_args() {
...
fn start_with_config_no_args() {
    let mut config = VersmConfig::default();
    config.managers.available = vec!["configured recipient".to_owned()];
    let expected_line = format!("Hello, {:?}!", &config.managers.available);
